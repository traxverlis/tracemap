...
# Bounded quantifiers (RFC 5321 lengths) keep the scan linear: an unbounded
# local part with no "@" would be re-scanned from every position, which is
# quadratic (polynomial ReDoS) on attacker-supplied text.
EMAIL_PATTERN = re.compile(r"(?<![\w.+-])[\w.+-]{1,64}@[\w-]{1,63}(?:\.[\w-]{1,63}){1,8}")
